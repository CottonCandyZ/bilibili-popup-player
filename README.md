...
构建会从最终脚本头生成根目录和 `dist` 中的 `.meta.js`，确保版本、名称、命名空间与下载文件一致；Cloudflare Pages 的 `dist/_headers` 为两个更新地址设置 `no-store, no-cache, max-age=0, must-revalidate`，避免缓存旧版。部署后需能通过上述两个公网地址直接取得脚本，不能返回安装页 HTML。
...
遇到缓存尚未刷新、网络失败、元数据与下载版本不一致或下载内容不完整时，校验会报错。校验也会检查正式域名实际返回的缓存头，拒绝缺少 `no-store` / `no-cache` 的响应，防止域名的缓存设置覆盖 Pages 配置后仍误报成功。检查部署和缓存状态后可重新执行该命令；它不会重新发布。
